...
export async function fetchExchangeRates(baseCurrency: CurrencyCode): Promise<ExchangeRates> {
  // Check cache
  if (cachedRates && cachedRates.base === baseCurrency && Date.now() - cacheTimestamp < CACHE_DURATION) {
    return cachedRates;
  }

  try {
    const symbols = ['EUR', 'USD', 'GBP', 'CHF', 'PLN', 'SEK', 'NOK', 'DKK'].filter(s => s !== baseCurrency);
