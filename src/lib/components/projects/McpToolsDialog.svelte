	...
	const TOOL_CATEGORIES = [
		{
			title: 'Project',
			tools: [
				{ name: 'list_branches', label: 'List Branches' },
				{ name: 'create_branch', label: 'Create Branch' }
			]
		},
		{
			title: 'Items',
			tools: [
				{ name: 'add_furniture_item', label: 'Add Item' },
				{ name: 'update_furniture_item', label: 'Update Item' },
				{ name: 'delete_furniture_item', label: 'Delete Item' },
				{ name: 'batch_add_items', label: 'Batch Add Items' },
				{ name: 'batch_update_items', label: 'Batch Update Items' },
				{ name: 'list_furniture_items', label: 'List Items' }
			]
		},
		{
			title: 'Images',
			tools: [
				{ name: 'add_item_image_from_url', label: 'Add Image from URL' },
				{ name: 'list_item_images', label: 'List Images' },
				{ name: 'delete_item_image', label: 'Delete Image' }
			]
		},
		{
			title: 'Preview',
			tools: [{ name: 'get_project_preview', label: 'Get Preview' }]
		},
		{
			title: 'Floorplan',
			tools: [
				{ name: 'save_floorplan_analysis', label: 'Save Analysis' },
				{ name: 'get_floorplan_analysis', label: 'Get Analysis' }
			]
		},
		{
			title: 'Spatial',
			tools: [
				{ name: 'get_room_contents', label: 'Room Contents' },
				{ name: 'get_available_space', label: 'Available Space' },
				{ name: 'check_placement', label: 'Check Placement' },
				{ name: 'suggest_placement', label: 'Suggest Placement' }
			]
		},
		{
			title: 'Comments',
			tools: [
				{ name: 'list_comments', label: 'List Comments' },
				{ name: 'add_comment_reply', label: 'Add Reply' }
			]
		}
	] as const;
							...
							{#each category.tools as tool}
								<div class="flex items-center justify-between py-1">
									<div class="flex flex-col">
										...
										<span class="text-xs text-slate-500 font-mono">{tool.name}</span>
									</div>
									<Switch
