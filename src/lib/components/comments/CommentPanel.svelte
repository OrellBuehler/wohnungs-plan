...
{#snippet pendingInput()}
	<div class="flex flex-col gap-2">
		<span class="text-xs text-outline">{m.comments_panel_type_label()}</span>
		<div class="flex flex-col gap-2">
			<textarea
...
{#snippet listView()}
	<div class="flex flex-col gap-3 h-full">
			...
			<div class="flex flex-col gap-2">
				...
				<div class="flex gap-2">
					<Button
						variant="outline"
						size="sm"
						class="flex-1 text-secondary border-secondary/30 hover:bg-secondary-fixed"
		...
		<button
			type="button"
			class="text-xs text-outline hover:text-on-surface-variant text-left"
		...
		<div class="flex-1 overflow-y-auto space-y-1.5 min-h-0">
			{#if filteredComments.length === 0}
				<div class="flex flex-col items-center justify-center py-8 text-center">
					<MessageCircle class="size-8 text-outline mb-2" />
					<p class="text-sm text-outline">{m.comments_panel_empty()}</p>
				</div>
			{:else}
				{#each filteredComments as comment (comment.id)}
					<button
						type="button"
						class="w-full text-left p-2 rounded-md hover:bg-surface transition-colors {comment.resolved
							? 'opacity-60'
							: ''}"
					...
					>
						<div class="flex items-start gap-2">
							<div
								class="w-6 h-6 rounded-xl flex items-center justify-center text-xs font-medium flex-shrink-0 {comment.resolved
							...
							<div class="flex-1 min-w-0">
								<div class="flex items-center gap-1.5">
									...
									<span class="text-xs text-outline flex-shrink-0"
										>{formatTime(comment.updatedAt ?? comment.createdAt)}</span
									>
								</div>
								<p class="text-xs text-on-surface-variant truncate">{getPreviewText(comment)}</p>
...
{#if open}
	<div
		class="{isMobile
			? 'w-full flex-1'
			: 'w-72 flex-shrink-0 h-full'} bg-surface-container-lowest p-4 flex flex-col gap-3 overflow-hidden"
	>
		<!-- Header -->
		<div class="flex items-center justify-between flex-shrink-0">
			...
			{:else}
				<span class="text-sm font-medium text-on-surface">{m.comments_panel_title()}</span>
			{/if}
			<div class="flex items-center gap-1">
				<Button
					variant="ghost"
					size="sm"
					class="h-7 w-7 p-0 {commentsVisible ? 'text-on-surface-variant' : 'text-outline'}"
