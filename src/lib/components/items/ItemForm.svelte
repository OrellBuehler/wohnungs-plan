<script lang="ts">
	...
	function handleSubmit(e: Event) {
		...
		const priceValue =
			typeof price === 'number' ? price : String(price).trim() ? parseFloat(String(price)) : null;
...
<Dialog.Root bind:open onOpenChange={(o) => !o && handleClose()}>
	...
	>
		<Dialog.Header>
			<Dialog.Title class="font-display">{item?.name ? m.item_form_edit_title() : m.item_form_add_title()}</Dialog.Title>
		</Dialog.Header>
		...
		<form onsubmit={handleSubmit} class="space-y-4 overflow-y-auto flex-1 min-h-0 px-0.5">
			...
			<div class="space-y-2">
				...
				<Input id="url" type="url" bind:value={productUrl} placeholder="https://..." />
			</div>
				...
				<div class="flex gap-2 flex-wrap">
					...
					{#each pendingPreviews as preview, i}
						...
						>
							<img
								src={preview}
								alt={m.item_form_image_pending_alt()}
